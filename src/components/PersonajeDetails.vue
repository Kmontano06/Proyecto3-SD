  ...
  <div class='container mt-5'>
    ...
    <form class='row g-3 mt-2'>
      ...
      <div class='col-12 text-r mb-5'>
        ...
        <button v-if='edit' class='btn btn-primary' v-on:click='updatePersonaje(personaje.id)'>Actualizar Personaje</button>
      </div>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getPersonajeN: function(){
      let num = 0;
      fetch(this.url+'/.netlify/functions/personajeN',
      { headers: {'Accept': 'application/json'}})
      .then((response) => response.json())
      .then((items) => {
        num = parseInt(items);
        this.personaje.id = "personaje_"+ (num+1);
      });
    },
